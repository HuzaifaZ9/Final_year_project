...
// Function to send video URLs to backend for analysis
function analyzeVideos() {
  // Create a queue of video URLs
  var videoQueue = videoData.map(function(video) {
    return video.url;
  });
  ...
  function processQueue() {
    if (videoQueue.length > 0) {
      var videoURL = videoQueue.shift(); // Get the first URL from the queue
      ...
      fetch('http://localhost:3000/api/videos', {
        ...
        body: JSON.stringify({ videoURL }), // Include video URL in the request body
      })
      .then(response => response.json())
      .then(result => {
        // Update the video stats in the DOM
        updateVideoStats(videoURL, result);
...
// Function to update video stats in the DOM
function updateVideoStats(videoURL, result) {
  var videoCard = document.querySelector(`[href="${videoURL}"]`).closest('.video-card'); // Find the parent video card
  if (videoCard) {
    var statsElement = videoCard.querySelector('.video-stats');
    statsElement.innerHTML = `
      <p>Context: ${result.context}</p>
      <p>Cognitive: ${result.cognitive}</p>
      <p>Music Stimulus: ${result.musicStimulus}/10</p>
      <p>Visual Stimulus: ${result.visualStimulus}/10</p>
      <p>Objects Used: ${result.objectsUsed}/10</p>
      <p>Habit Forming Nature: ${result.habitFormingNature}/10</p>
      <p>Story Moral: ${result.storyMoral}/10</p>
      <p>Age Group: ${result.ageGroup}</p>
    `;
  }
}

// Call the function to send video URLs for analysis
analyzeVideos();
